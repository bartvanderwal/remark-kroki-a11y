...
We use PlantUML's data structure as internal representation (IR) with adapters for each input format. See [ADR-0006](./adr/0006-plantuml-als-interne-standaard.md) for details.
...
For all architecture decisions, see the [Architecture Decision Records (ADRs)](./adr/).
...
![Strategy Pattern class diagram example](./img/strategy-pattern-example.svg)
...
Want to help? Or update and run this plugin itself? See [CONTRIBUTING.md](./contributing) for local development setup and publishing tips.

For quality standards and acceptance criteria, see the [Definition of Done](./definition-of-done).
