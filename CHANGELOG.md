...
- Detect `remark-kroki` fail SVG output and expose it as a typed
  `DiagramRenderError` instead of letting downstream consumers parse fallback
  SVGs.
- Added `throwOnDiagramError` and `onDiagramError` options for consumers that
  want either fail-fast builds or custom render-error handling.
